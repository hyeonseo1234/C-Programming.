...
void Excersize_3_25(int solution, int answer) //한라대학교 미래모빌리티공학과 신현서
{
    ...
    if (solution == SHOW)
    {
        ...
        printf("y = 나중높이, y0 : 처음높이, v0y : y의 처음속도, g : 중력가속도, t : 시간\n\n");
        ...
        printf("0 = %6.2lft^2 + %6.2lft + %6.2lf\n\n", a, b, c);
    ...
    if (solution == SHOW)
    {
        ...
        printf("Δx = 거리변화량, v0x : x의 처음속도, t : 시간\n\n");

        delta_x = velocity_0 * (cos(degree * PI / 180)) * time_1;

        printf("Δx = v0x * t\n");
        printf("    = v0 * cosθ * t\n\n");
        printf("Δx = %6.2lf m/s^2 * cos%6.2lf˚ * %6.2lf\n", velocity_0, degree, time_1);
        printf("    = %6.2lf m/s^2\n\n", delta_x);
    }
